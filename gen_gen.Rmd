...
# 1000 поколений = 70 секунд, в файле RData 380 Кбайт
```{r}
for (param.id_gen in 3:1000) {
  gen.local <- gen_generation(param.id_gen, 100, gen_last)
  gen_last <- gen.local
  gen_all = rbind(gen_all, gen.local)
}
```
...
## График, на кот. показано, что сколько особей первого поколения не имеют потомков в этом поколении
